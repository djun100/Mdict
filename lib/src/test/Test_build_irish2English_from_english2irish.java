...
public class Test_build_irish2English_from_english2irish {
    ...
    public static void main(String[] args) throws IOException{
    	...
    	//UtilMy.show(md.getEntryAt(1347));
    	//UtilMy.show(md.getRecordAt(1347));
    	//md.printDictInfo();
    	...
    	if(true) {
	    	...
	    	for(int i=0;i<md.getNumberEntries();i++) {
	    		...
	    		//UtilMy.show(retroContent);
				/*
    			if(res2.contains("`2`")) {
    				fOut.write(res);
    				//UtilMy.show("|"+i+"|"+res);
    				//UtilMy.show(entrI.trim());
    			}
				 */	
    	...
    	//UtilMy.show(md2.getEntryAt(1328));
    	UtilMy.show(md2.getRecordAt(13280));
    	//md.printAllKeys();//md2.lookUp("úth")
    	//TODO entry过长时会出错 record
    	//UtilMy.show(md2.getRecordAt(md2.lookUp("rite")));
    	//UtilMy.show(md2.getEntryAt(32000));//TODO add border check
